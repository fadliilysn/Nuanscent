...
class NuanscentPerfumeCleanBatch04Seeder extends Seeder
{
        ...
        DB::transaction(function (): void {
            ...
            foreach ($this->payload['perfumes'] as $perfumeData) {
                ...
                if (array_key_exists('variants', $perfumeData)) {
                    ...
                    if ($this->hasVariantPrices($variants)) {
    ...
    private function readPayload(): array
    {
        ...
        if (! is_array($payload) || ! isset($payload['brands'], $payload['perfumes'])) {
            throw new RuntimeException('Format dataset Clean Batch 04 tidak valid: key brands dan perfumes wajib tersedia.');
        }

        return $payload;
    }

    private function validateCleanCategoryReferences(): void
    {
        $unexpectedCategories = collect($this->collectPerfumeValues('main_aroma_category_slug'))
            ->reject(fn (string $slug): bool => $slug === self::EXPECTED_CATEGORY_SLUG)
            ->sort()
            ->values()
            ->all();

        if ($unexpectedCategories !== []) {
            throw new RuntimeException('Dataset Clean Batch 04 hanya boleh memakai main_aroma_category_slug clean. Slug tidak sesuai: '.implode(', ', $unexpectedCategories));
        }
    }
    ...
    private function validateBrandReferences(): void
    {
        $missingBrands = $this->missingSlugs(
            $this->collectPerfumeValues('brand_slug'),
     ...
     */
    private function existingBrands(): array
    {
        return Brand::query()
            ->whereIn('slug', $this->collectPerfumeValues('brand_slug'))
            ->get()
            ->keyBy('slug')
            ->all();
    }
     ...
     * @param  array<int, array<string, mixed>>  $variants
     */
    private function hasVariantPrices(array $variants): bool
    {
        return collect($variants)
            ->contains(fn (array $variant): bool => array_key_exists('price', $variant) && $variant['price'] !== null);
    }
