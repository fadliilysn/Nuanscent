...
class NuanscentPerfumeBatch02Seeder extends Seeder
{
    ...
    public function run(): void
    {
        ...
        $this->command?->info(
            "Batch 02 import selesai: {$this->counts['brands']} brand, {$this->counts['perfumes']} parfum, {$this->counts['notes']} note unik, {$this->counts['variants']} varian disinkronkan.",
        );
    }
    ...
    private function readPayload(): array
    {
        ...
        if (! is_array($payload) || ! isset($payload['brands'], $payload['perfumes'])) {
            throw new RuntimeException('Format dataset Batch 02 tidak valid: key brands dan perfumes wajib tersedia.');
        }
